...
export default function ConversationPage() {
  ...
  const flyingOutbound = messages.find(
    ({ message, delivery }) =>
      message.sender_id === user?.id &&
      delivery &&
      ['DISPATCHED', 'PREPARING', 'FLYING', 'ARRIVED'].includes(delivery.status)
  );
  ...
  return (
    <div className="page page--chat">
      <div className="chat-header">
        <button type="button" onClick={() => navigate('/inbox')} aria-label="Back to messages" className="icon-chip">
          ←
        </button>
        <PigeonAvatar spriteId={peerSprite} size={42} name={peer?.display_name} animate={false} />
        <div style={{ minWidth: 0, flex: 1 }}>
          <strong style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {peer?.display_name || 'Conversation'}
          </strong>
          <span className="caption">{peer?.pigeon_id || ''}</span>
        </div>
      </div>

      <div className="chat-thread">
        {loading && <p className="muted" style={{ textAlign: 'center', padding: 20 }}>Loading…</p>}
        ...
        {!loading && messages.length === 0 && (
          <div className="empty-state">
            ...
            <p className="caption">Send the first pigeon below.</p>
          </div>
        )}

        {flyingOutbound?.delivery && (
          <div className="flight-status">
            <strong>Delivery on the way</strong>
            <p className="caption" style={{ marginTop: 4 }}>
              {messageStatus(flyingOutbound.message, flyingOutbound.delivery, user?.id || '')}
            </p>
            <div className="flight-track">
              <div
                className="fill"
                style={{ width: `${estimatedProgressPercent(flyingOutbound.delivery)}%` }}
              />
              <span
                className="bird"
                style={{ left: `${estimatedProgressPercent(flyingOutbound.delivery)}%` }}
              >
                🐦
              </span>
            </div>
            <Link
              to={`/delivery/${flyingOutbound.delivery.id}`}
              ...
              return (
                <div key={message.id} className={`bubble-row ${mine ? 'mine' : ''}`}>
                  <div className={`bubble ${mine ? 'mine' : ''}`}>
                    ...
                    <div className="meta">
                      <span>{formatSmartTime(message.created_at)}</span>
      ...
      {preview && (
        <div className="caption" style={{ padding: '5px 4px', textAlign: 'center' }}>
      ...
      {error && <p className="error-text" style={{ margin: '4px 0 6px' }}>{error}</p>}
      ...
      <div className="chat-composer">
        ...
        <button
          type="button"
          className="btn btn-primary send-fab"
          disabled={!canSend}
