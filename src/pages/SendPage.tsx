...
export default function SendPage() {
  ...
  return (
    <div className="page">
      ...
      {step === 1 && (
        <>
          <p className="muted" style={{ marginBottom: 12 }}>Choose a friend to start a chat</p>
          {friends.length === 0 && (
            <div className="card empty-state">
              <div className="emoji">👥</div>
              <p>No friends yet. Add someone first.</p>
            </div>
          )}
          {friends.map((f) => (
            <button
            ...
            >
              <UserAvatar name={f.display_name} src={f.avatar_url} size={48} />
              <div style={{ minWidth: 0 }}>
                <strong>{f.display_name}</strong>
                <div className="caption">@{f.username} · {f.pigeon_id}</div>
              </div>
            </button>
          ))}
        </>
