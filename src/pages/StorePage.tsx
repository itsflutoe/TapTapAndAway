...
type StoreTab = 'pigeons' | 'items' | 'special';
...
export default function StorePage() {
  ...
  const special = catalog.filter((c) => c.is_featured && c.item_type !== 'bird');
  ...
  return (
    <div className="page">
      ...
      >
        {(
          ...
          <button
            ...
            style={{ flex: 1, minWidth: 90, fontSize: 13, padding: '8px 10px' }}
            onClick={() => setTab(id)}
          >
            {label}
          </button>
      ...
      {tab === 'special' && (
        <div className="card" style={{ marginBottom: 16 }}>
          <h2 style={{ fontSize: 16, marginTop: 0 }}>Special & events</h2>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 0 }}>
            Featured finds and limited offerings.
          </p>
          {special.length === 0 && (
            <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
              Nothing special in stock right now.
            </p>
          )}
          {special.map(renderCard)}
        </div>
      )}
