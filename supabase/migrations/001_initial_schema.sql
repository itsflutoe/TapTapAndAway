...
BEGIN
  ...
  v_address := COALESCE(NEW.raw_user_meta_data->>'address', 'Unknown');
  ...
  IF v_gender = 'female' THEN v_pigeon_gender := 'female';
  ELSIF v_gender = 'male' THEN v_pigeon_gender := 'male';
  ELSE v_pigeon_gender := CASE WHEN random() < 0.5 THEN 'male' ELSE 'female' END;
  END IF;

  SELECT COALESCE((value::text)::integer, 10) INTO v_bonus FROM public.system_settings WHERE key = 'signup_stamp_bonus';

  INSERT INTO public.profiles (id, username, display_name, gender, address, latitude, longitude, stamp_balance)
  VALUES (NEW.id, v_username, v_display_name, v_gender, v_address, v_lat, v_lng, v_bonus);

  INSERT INTO public.pigeons (owner_id, name, gender, speed, stamina, reliability)
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'pigeon_name', 'Mochi'), v_pigeon_gender, 100.00, 50.00, 95.00);
